  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/newsletter', {
        ...
        body: JSON.stringify({ email: email.trim(), source: 'sparklearn' }),
      })
    ...
    <section className="w-full bg-white p-2 sm:p-3 md:p-3.5 lg:p-4 xl:p-5">
      <div className="relative rounded-2xl md:rounded-[24px] bg-[#070709] text-white overflow-hidden p-4 sm:p-6 lg:p-6 xl:p-8 min-h-[520px] sm:min-h-[580px] lg:min-h-0 lg:h-[calc(100vh-2rem)] lg:max-h-[800px] xl:max-h-[850px] flex flex-col justify-between shadow-2xl border border-white/10">
        ...
        <div className="relative z-10 my-auto w-full max-w-3xl xl:max-w-4xl mx-auto text-center py-1 sm:py-2 flex flex-col items-center">
                  ...
                  >
                    <div className="rounded-full border border-white/20 bg-black/60 hover:border-white/35 focus-within:border-[#EB4604] p-1.5 pl-3.5 sm:pl-5 flex items-center justify-between transition-all duration-300 shadow-xl w-full max-w-full">
                      ...
                      <button
                        type="submit"
                        disabled={loading}
                        className="group px-3.5 sm:px-5 py-2 rounded-full bg-[#EB4604] hover:bg-[#D43D00] text-white text-xs sm:text-xs xl:text-sm font-semibold transition-all duration-300 hover:scale-[1.02] shadow-md shadow-[#EB4604]/30 active:scale-[0.98] shrink-0 inline-flex items-center justify-center gap-1.5 select-none disabled:opacity-60 cursor-pointer"
                      >
                        {loading ? (
                          <>
          ...
          {/* Minimalist & Modern Feature Badges (Static & Clean) */}
          <RevealOnScroll delay={0.25}>
            <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-2.5">
              {[
                ...
                <div
                  key={idx}
                  className="inline-flex items-center gap-2 px-3 sm:px-3.5 py-1.5 rounded-full bg-white/[0.04] border border-white/[0.08] backdrop-blur-md shadow-[inset_0_1px_0_rgba(255,255,255,0.10)] select-none"
                >
                  <span className="flex items-center justify-center shrink-0">
                    {item.icon}
                  </span>
                  <span className="text-[11.5px] sm:text-xs font-medium tracking-tight text-neutral-300">
                    {item.label}
                  </span>
                </div>
              ))}
