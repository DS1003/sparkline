...
export function Tag({ children, variant = 'base', size = 'default', className }: TagProps) {
  ...
  return (
    <div
      className={cn(
        'inline-flex items-center rounded-full select-none transition-all duration-300 group cursor-default whitespace-nowrap max-w-full',
        isSmall
          ? 'gap-1.5 sm:gap-2 md:gap-2.5 px-2.5 sm:px-4 md:px-5 py-1 sm:py-2 md:py-2.5'
          : 'gap-2 sm:gap-2 md:gap-2.5 px-3.5 sm:px-4 md:px-5 py-1.5 sm:py-2 md:py-2.5',
        isDarkBg
    ...
    >
      ...
      <span
        className={cn(
          'relative flex shrink-0 items-center justify-center transition-transform duration-300 group-hover:rotate-12',
          isSmall
            ? 'h-2.5 w-2.5 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4'
            : 'h-3.5 w-3.5 sm:h-3.5 sm:w-3.5 md:h-4 md:w-4'
        )}
      >
        <svg
          viewBox="0 0 884 884"
      ...
      <span
        className={cn(
          'font-mono font-semibold uppercase leading-none whitespace-nowrap',
          isSmall
            ? 'text-[8.5px] sm:text-[11px] md:text-[12.5px] tracking-[0.04em] sm:tracking-[0.12em]'
            : 'text-[10.5px] sm:text-[11px] md:text-[12.5px] tracking-[0.08em] sm:tracking-[0.12em]'
        )}
      >
        {children}
      </span>
